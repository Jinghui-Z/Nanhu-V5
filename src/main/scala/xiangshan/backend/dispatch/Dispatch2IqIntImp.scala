...
class Dispatch2IqIntImp(override val wrapper: Dispatch2Iq)(implicit p: Parameters, params: SchdBlockParams)
  ...
  println(s"[Dispatch2IqIntImp] fuConfigOnlyOneIQ: ${fuConfigOnlyOneIQ.map(_.name)}")
  println(s"[Dispatch2IqIntImp] fuConfigMultiIQ: ${fuConfigMultiIQ.map(_.name)}")
  ...
  val minIQSel = VecInit(fuConfigMultiIQ.map{ case fuConfig =>
    println("fu: ", fuConfig)
    val thisFuValidNumSeq = VecInit(exeFuConfigs.zip(io.intIQValidNumVec.get.flatten).filter{case (a,b) => a.contains(fuConfig)}.map(_._2))
    println(thisFuValidNumSeq.length)
    ...
    selIQOHSeq.zipWithIndex.map{ case (s, i) =>
      s.zipWithIndex.map{ case (ss, j) =>
        if (IQFuConfigs(j).contains(fuConfig)) {
          ss := sortResult(i % sortResult.size)(j % thisFuValidNumSeq.length)
        } 
        else ss := false.B
      }
    }
  ...
  if(backendParams.debugEn){
    dontTouch(io.intIQValidNumVec.get)
